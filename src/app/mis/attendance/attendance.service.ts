...
export class AttendanceService {
  ...
  loadStream(id: number): Observable<any> {
    return this.http._get(`${this.api}/attendance/umarked?category=stream&id=${id}`);
  }

  loadClass(id: number): Observable<any> {
    return this.http._get(`${this.api}/attendance/umarked?category=class&id=${id}`);
  }
  ...
  getStreamAttendance(stream, date): Observable<any> {
    return this.http._get(`${this.api}/attendance/stream?stream=${stream}&date=${date}`);
  }

  getClassAttendance(clas, date): Observable<any> {
    return this.http._get(`${this.api}/attendance/class?class=${clas}&date=${date}`);
  }

  getStudentAttendance(adm, startDate, endDate): Observable<any> {
    return this.http._get(`${this.api}/attendance/student?adm=${adm}&startDate=${startDate}&endDate=${endDate}`);
  }
